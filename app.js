...
const normalize = (v) => String(v || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/ł/g, "l");
...
function isLeadershipRole(role) {
  ...
  return value.includes("dyrektor") || value.includes("naczelnik");
}

function personBranchLabels(person) {
  const records = Array.isArray(person._records) && person._records.length ? person._records : [person];
  const labels = new Set();
  records.forEach(record => {
    const code = String(record.code || "").trim().toUpperCase();
    const location = normalizeOrgLocation([record.path, record.unit].filter(Boolean).join(" "));
    if (code === "BBI" || location.includes("filia wup w bielsku-bialej")) labels.add("Filia Bielsko-Biała");
    if (code === "BCZ" || location.includes("filia wup w czestochowie")) labels.add("Filia Częstochowa");
  });
  return [...labels];
}
...
function personCard(p) {
  ...
  const isLeadershipPerson = assignments.some(item => isLeadershipRole(item.role));
  const branchLabels = personBranchLabels(p);
  const branchBadges = branchLabels.length
    ? `<div class="branch-badges">${branchLabels.map(label => `<span class="branch-badge">${esc(label)}</span>`).join("")}</div>`
    : "";
  const assignmentHtml = assignments
    .filter(item => item.role || item.unit || item.code)
    .map(item => {
      const leadershipClass = isLeadershipRole(item.role) ? " leadership-assignment" : "";
      return `<div class="assignment${leadershipClass}">${item.role ? `<div class="role">${esc(item.role)}</div>` : ""}${item.unit ? `<div class="unit">${esc(item.unit)}</div>` : ""}</div>`;
    })
    .join("");
  ...
  return `<article class="card${isLeadershipPerson ? " leadership-card" : ""}${branchLabels.length ? " branch-card" : ""}"><div class="person-top"><div class="avatar">${esc(initials(p.name))}</div><div class="person-details"><div class="person-title-row"><div class="name">${esc(p.name)}</div>${branchBadges}</div><div class="assignments">${assignmentHtml}</div></div></div><div class="meta">${phoneChips}${emailChips}${roomChips}${codeChips}</div></article>`;
}
